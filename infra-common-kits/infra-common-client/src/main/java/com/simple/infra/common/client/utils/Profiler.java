...
public final class Profiler {
  ...
  /*public static void dump(Logger logger) {
    ...
    public static void dump(String logTag, Logger logger) {
        Entry entry = ENTRY_STACK.get();
        if (entry == null) {
            return;
        }
        entry.dump(logTag, logger, "", "");
    }
    ...
    public static final class Entry {
        ...
        public String getMessage() {
            ...
            if (message instanceof String) {
                ...
                messageString = ((Message) message).dump();
            }
        ...
        public void dump(String logTag, Logger logger, String prefix1, String prefix2) {
            ...
            StringBuilder pattern = new StringBuilder(prefix1);
            ...
            Logs.info(logTag, logger, MessageFormat.format(pattern.toString(), params));

            for (int i = 0; i < subEntries.size(); i++) {
                Entry subEntry = subEntries.get(i);
                if (i == (subEntries.size() - 1)) {
                    subEntry.dump(logTag, logger, prefix2 + "`---", prefix2 + "    "); // 最后一项
                } else if (i == 0) {
                    subEntry.dump(logTag, logger, prefix2 + "+---", prefix2 + "|   "); // 第一项
                } else {
                    subEntry.dump(logTag, logger, prefix2 + "+---", prefix2 + "|   "); // 中间项
                }
            }
        }
